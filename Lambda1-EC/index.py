...
EC_MetricName = os.environ['MetricName']
...
#   List metrics through the pagination interface
    ...
    CW_filteredIterator = CW_iterator.search("MetricAlarms[?MetricName==`"+EC_MetricName+"` && Namespace==`AWS/ElastiCache`]")
...
#   Prepare ElastiCache clusters full list
    ...
    for EC_response in EC_iterator:
        EC_rawList.append(EC_response['CacheClusters'])
...
#       判断已有的监控告警是否为正准备创建的监控告警
        if alarm['MetricName'] == EC_MetricName:
            for dimension in alarm["Dimensions"]:
                if dimension["Name"] == "CacheClusterId":
                    EC_ignoreList.append(dimension["Value"])
    print(EC_ignoreList)


#   Drop CloudWatch alarm cascade: 
    print('------')
    for cw in EC_ignoreList:
        if is_existed_inList(cw, EC_nameList):
            print('Dropping CloudWatch alarm "EC_'+EC_MetricName+'" for: '+cw)
            CWalarms = CW_client.delete_alarms(
                AlarmNames=['EC_'+EC_MetricName+'-'+cw]
            )
            print('Dropped CloudWatch alarm "ES_'+EC_MetricName+'" for: '+cw)
...
#   预先定义SNS topic suffix，不同的CloudWatch告警通知会发送到不同的SNS topic
    for ec_id in EC_idList:
        ...
        print('------')
#       判断是否并未创建对应的监控告警
        if is_existed_inList(ec_name, EC_ignoreList):
...
#           创建监控告警
            CWalarms = CW_client.put_metric_alarm(
                AlarmName='EC_'+EC_MetricName+'-'+ec_name,
                AlarmDescription='Auto-created customized CloudWatch Alarm <EC_'+EC_MetricName+'>',
                ActionsEnabled=True,
#                OKActions=[
#                    'string',
#                ],
...
#           为该EC集群标记CWalarm-DatabaseMemoryUsagePercentage标签
            EC_tags = EC_client.add_tags_to_resource(
                ...
                Tags = [
                    {
                        'Key': str('CWalarm-'+EC_MetricName),
                        'Value': 'enabled'
                    },
                ]
            )
            print('Added tag "CWalarm-'+EC_MetricName+'" to: '+ec_name)
                    
            print('Created CloudWatch alarm "EC_'+EC_MetricName+'" for Cluster <'+ec_name+'>')
            print()
        else: 
